...
BLE screen-off capture is designed around the `iot` submodule and Seeed Studio XIAO ESP32S3 Sense. Mobile BLE uses `react-native-ble-plx`, so scanning and subscription require a native development build rather than Expo Go or web.
