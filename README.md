...
XIAO ESP32S3 Sense 固件在 `iot/iot.ino`。烧录后设备会以 `Stardust Sense` 名称广播，移动端可在设置页扫描、订阅事件、同步 manifest，并按设备声明的 `capabilities` 启用捕获、同步和休眠命令。

设备事件会进入本地 `device_events` 和 `episodes`：

- `capture`、`button`、`serial` 事件可以在 Memory Inbox 中转入待确认记忆。
- `manifest`、`command`、`connection` 事件作为运行审计保留，用于排查配对、命令发送和断线重连状态。
- 大文件、图片和音频传输暂不走 BLE；固件 manifest 中只保留 microSD / media 占位信息，后续再接 Wi-Fi 局域网传输。
