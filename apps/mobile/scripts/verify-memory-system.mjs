...
assertIncludes(journalScreen, 'entry.source === "memory" || entry.source === "reflection"', "Memory and reflection timeline entries must expose graph navigation.");
assertIncludes(journalScreen, "params: { nodeId: entry.nodeId }", "Memory timeline navigation must target the selected graph node.");
assertIncludes(journalScreen, 'entry.source !== "memory" && entry.source !== "reflection"', "Episode timeline entries must expose source navigation.");
